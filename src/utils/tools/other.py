...
retriever = VectorIndexRetriever(
    index=vector_index, similarity_top_k=15)
...
class CustomNodePostprocessor(BaseNodePostprocessor, BaseModel):
    ...
    def _init_(self, filter_query: str, **kwargs):
    ...
    def _postprocess_nodes(
        ...
        # Filter nodes based on the company_name metadata
        filtered_nodes = [
            node for node in nodes
            if node.node.metadata.get("company_name", "").lower() == self.company_name
        ]
